...
export function WhatIDo() {
  ...
  return (
    <section id="whatido" className="py-24 px-6 md:px-12">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8">
          {cards.map((card, index) => (
            <div 
              key={index} 
              className="glass-card p-10 group hover:-translate-y-2 hover:border-[var(--color-accent)]/50 transition-all duration-300"
            >
              <div className="w-16 h-16 rounded-2xl bg-[var(--color-primary)] flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300 border border-[var(--color-border)]">
                {card.icon}
              </div>
